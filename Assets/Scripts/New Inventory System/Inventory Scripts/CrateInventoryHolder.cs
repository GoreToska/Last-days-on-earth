...
public class CrateInventoryHolder : InventoryHolder, IInteractable
{
	...
	public void HighlightInteracable(bool value)
	{
		if (value)
		{
			...
			{
				renderer.materials[renderer.materials.Length - 1].SetColor("_Color", _interactableHighlightConfig.Highlight);
				renderer.materials[renderer.materials.Length - 1].SetFloat("_Scale", 1.03f);
			}
		}
		else
		{
			foreach (var renderer in _meshRenderer)
			{
				renderer.materials[renderer.materials.Length - 1].SetFloat("_Scale", 0f);
			}
		}
	}
	public void HighlightCurrentInteractable(bool value)
	{
		if (value)
		{
			foreach (var renderer in _meshRenderer)
			{
				renderer.materials[renderer.materials.Length - 1].SetColor("_Color", _interactableHighlightConfig.ChosedHighlight);
				renderer.materials[renderer.materials.Length - 1].SetFloat("_Scale", 1.03f);
			}
		}
		else
		{
			foreach (var renderer in _meshRenderer)
			{
				renderer.materials[renderer.materials.Length - 1].SetFloat("_Scale", 1f);
			}
		}
	}
